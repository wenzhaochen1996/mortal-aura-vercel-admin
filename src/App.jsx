...
function BasicInput({ label, value, onChange, type = "text", placeholder = "" }) {
  return (
    <label className="space-y-2 block">
      <div className="text-[11px] uppercase tracking-[0.24em] text-zinc-500">{label}</div>
      <input type={type} value={value} placeholder={placeholder} onChange={(e) => onChange(e.target.value)} className="w-full rounded-2xl border border-white/10 bg-zinc-950 px-3 py-2 text-sm text-zinc-100 outline-none" />
    </label>
  );
}

function BasicTextarea({ label, value, onChange, rows = 3, placeholder = "" }) {
  return (
    <label className="space-y-2 block">
      <div className="text-[11px] uppercase tracking-[0.24em] text-zinc-500">{label}</div>
      <textarea value={value} rows={rows} placeholder={placeholder} onChange={(e) => onChange(e.target.value)} className="w-full rounded-2xl border border-white/10 bg-zinc-950 px-3 py-2 text-sm text-zinc-100 outline-none" />
    </label>
  );
}
...
export default function App() {
  ...
  const [checkoutForm, setCheckoutForm] = useState({ name: "", email: "", country: "Japan", city: "", address: "", note: "", phone: "", postalCode: "", region: "", address2: "" });
  ...
  const [accountData, setAccountData] = useState({ profile: null, addresses: [], orders: [] });
  const [accountLoading, setAccountLoading] = useState(false);
  const [checkoutLoading, setCheckoutLoading] = useState(false);
  const [addressForm, setAddressForm] = useState({ label: "Default", recipient_name: "", phone: "", country: "Japan", state_region: "", city: "", postal_code: "", address_line1: "", address_line2: "", is_default: true });
  ...
  useEffect(() => {
    ...
    loadAccountData(session.access_token);
  ...
  const addWorkToList = (listKey) => {
    setSiteContent((prev) => {
      const currentList = Array.isArray(prev?.[listKey]) ? prev[listKey] : [];
      const nextWork = ensureWorkShape({
        sku: `MA-NEW-${String(currentList.length + 1).padStart(3, "0")}`,
        title: ensureML("New Work"),
        subtitle: ensureML("New Entry"),
        collection: ensureML(listKey === "works" ? "Main Collection" : listKey === "oneOfAKindWorks" ? "One of a Kind" : listKey === "memoryWorks" ? "My Memories" : "For Your Lover"),
        collection2: ensureML(""),
        categoryKey: "Pendant",
        category: ensureML(prev?.categoryLabels?.Pendant ?? CATEGORY_OPTIONS.Pendant),
        material: ensureML("Silver 925"),
        stone: ensureML(""),
        size: ensureML(""),
        description: ensureML(""),
        homeExcerpt: ensureML(""),
        image: "",
        images: [],
        price: "¥0",
        stock: "1",
  ...
  const updateWorkList = (listKey, index, patch) => {
    setSiteContent((prev) => ({
      ...prev,
      [listKey]: (Array.isArray(prev?.[listKey]) ? prev[listKey] : []).map((item, i) => (i === index ? { ...item, ...patch } : item)),
    }));
  };

  const loadAccountData = async (token = session?.access_token) => {
    if (!token) return;
    ...
    try {
      ...
      setCheckoutForm((prev) => ({
        ...prev,
        name: data.profile?.full_name || defaultAddress?.recipient_name || prev.name,
        email: data.profile?.email || session?.user?.email || prev.email,
        country: defaultAddress?.country || prev.country,
        city: defaultAddress?.city || prev.city,
        address: defaultAddress?.address_line1 || prev.address,
        address2: defaultAddress?.address_line2 || prev.address2,
        phone: defaultAddress?.phone || prev.phone,
        postalCode: defaultAddress?.postal_code || prev.postalCode,
        region: defaultAddress?.state_region || prev.region,
      }));
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleAuthSubmit = async () => {
    ...
    if (authMode === 'login') {
      ...
      setAuthStatus(error ? error.message : 'Logged in');
  ...
  const handleLogout = async () => {
    ...
    setAccountData({ profile: null, addresses: [], orders: [] });
  };
  ...
  const saveProfile = async () => {
    ...
    const data = await apiFetch('/api/account-profile', {
      ...
      body: { full_name: checkoutForm.name },
    });
    setAccountData((prev) => ({ ...prev, profile: data.profile }));
  ...
  const saveAddress = async () => {
    ...
    setAddressForm({ label: 'Default', recipient_name: '', phone: '', country: 'Japan', state_region: '', city: '', postal_code: '', address_line1: '', address_line2: '', is_default: true });
    await loadAccountData(session.access_token);
  };
  ...
  const beginStripeCheckout = async () => {
    ...
    try {
      const payload = {
        email: checkoutForm.email || session?.user?.email || '',
        ...
        shipping_address: {
          ...
          state_region: checkoutForm.region,
          city: checkoutForm.city,
          postal_code: checkoutForm.postalCode,
          address_line1: checkoutForm.address,
          address_line2: checkoutForm.address2,
        },
      };
      const token = session?.access_token || '';
      const data = await apiFetch('/api/create-checkout-session', { method: 'POST', token, body: payload });
  ...
  return (
    <div className={`min-h-screen bg-black text-zinc-200 ${previewShiftClass}`}>
          ...
          <div className="rounded-[1.75rem] border border-white/10 bg-black/45 px-4 py-3 shadow-[0_12px_40px_rgba(0,0,0,0.22)] sm:px-5 md:px-6">
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex min-w-0 items-center gap-2 sm:gap-3">
                ...
                <button onClick={() => session ? setAccountOpen(true) : setAuthOpen(true)} className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-zinc-950 px-3 py-2 text-sm text-zinc-200 transition hover:border-white/30 hover:text-white sm:px-4">
                  <User size={16} />
              ...
              <div className="flex h-full flex-col">
                ...
                <div className="flex-1 space-y-4 overflow-y-auto px-4 py-4">
                  ...
                  <AdminSection title="作品板块（主作品）" defaultOpen>
                    <div className="space-y-4">
                      <button type="button" onClick={() => addWorkToList("works")} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-white/10 bg-zinc-900 px-4 py-3 text-sm text-zinc-100 transition hover:border-white/30 hover:bg-zinc-800">
                        <Plus size={16} /> 新增主作品
                      </button>
                  ...
                  <AdminSection title="一点物作品板块">
                    <div className="space-y-4">
                      <button type="button" onClick={() => addWorkToList("oneOfAKindWorks")} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-white/10 bg-zinc-900 px-4 py-3 text-sm text-zinc-100 transition hover:border-white/30 hover:bg-zinc-800">
                        <Plus size={16} /> 新增一点物作品
                      </button>
                  ...
                  <AdminSection title="记忆分区作品板块">
                    <div className="space-y-4">
                      <button type="button" onClick={() => addWorkToList("memoryWorks")} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-white/10 bg-zinc-900 px-4 py-3 text-sm text-zinc-100 transition hover:border-white/30 hover:bg-zinc-800">
                        <Plus size={16} /> 新增记忆分区作品
                      </button>
                  ...
                  <AdminSection title="Lover 分区作品板块">
                    <div className="space-y-4">
                      <button type="button" onClick={() => addWorkToList("loverWorks")} className="inline-flex w-full items-center justify-center gap-2 rounded-2xl border border-white/10 bg-zinc-900 px-4 py-3 text-sm text-zinc-100 transition hover:border-white/30 hover:bg-zinc-800">
                        <Plus size={16} /> 新增 Lover 分区作品
                      </button>
              ...
              <div className="space-y-6">
                <div className="rounded-3xl border border-white/10 bg-black/30 p-5">
                  <div className="grid gap-4 md:grid-cols-2">
                    <BasicInput label={ui.fullName} value={checkoutForm.name} onChange={(v) => setCheckoutForm((prev) => ({ ...prev, name: v }))} />
                    <BasicInput label={ui.email} value={checkoutForm.email} onChange={(v) => setCheckoutForm((prev) => ({ ...prev, email: v }))} type="email" />
                    <BasicInput label={ui.country} value={checkoutForm.country} onChange={(v) => setCheckoutForm((prev) => ({ ...prev, country: v }))} />
                    <BasicInput label={ui.city} value={checkoutForm.city} onChange={(v) => setCheckoutForm((prev) => ({ ...prev, city: v }))} />
                    <BasicInput label="Phone" value={checkoutForm.phone} onChange={(v) => setCheckoutForm((prev) => ({ ...prev, phone: v }))} />
                    <BasicInput label="Postal Code" value={checkoutForm.postalCode} onChange={(v) => setCheckoutForm((prev) => ({ ...prev, postalCode: v }))} />
                    <BasicInput label="State / Region" value={checkoutForm.region} onChange={(v) => setCheckoutForm((prev) => ({ ...prev, region: v }))} />
                  </div>
                  <div className="mt-4"><BasicTextarea label={ui.address} value={checkoutForm.address} onChange={(v) => setCheckoutForm((prev) => ({ ...prev, address: v }))} rows={3} /></div>
                  <div className="mt-4"><BasicTextarea label="Address line 2" value={checkoutForm.address2} onChange={(v) => setCheckoutForm((prev) => ({ ...prev, address2: v }))} rows={2} /></div>
                  <div className="mt-4"><BasicTextarea label={ui.orderNote} value={checkoutForm.note} onChange={(v) => setCheckoutForm((prev) => ({ ...prev, note: v }))} rows={4} /></div>
                </div>
                <div className="flex flex-wrap gap-3">
                  {session ? <button onClick={saveProfile} className="inline-flex items-center gap-2 rounded-2xl border border-white/10 px-5 py-3 text-sm text-zinc-100 transition hover:border-white/30 hover:text-white">Save profile</button> : <button onClick={() => setAuthOpen(true)} className="inline-flex items-center gap-2 rounded-2xl border border-white/10 px-5 py-3 text-sm text-zinc-100 transition hover:border-white/30 hover:text-white">Login to save details</button>}
                  ...
                  <button onClick={beginStripeCheckout} disabled={checkoutLoading} className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-zinc-100 px-5 py-3 text-sm text-black transition hover:opacity-90 disabled:opacity-60">
                    {checkoutLoading ? 'Processing...' : 'Pay securely'}
                  </button>
                </div>
              </div>
              <div className="space-y-4">
      ...
      {authOpen ? (
        <div className="fixed inset-0 z-[75] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm" onClick={() => setAuthOpen(false)}>
          <div className="w-full max-w-md rounded-[2rem] border border-white/10 bg-zinc-950 p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-sm font-medium uppercase tracking-[0.3em] text-white">{authMode === 'login' ? 'Customer Login' : 'Create Account'}</p>
                <p className="mt-1 text-xs text-zinc-500">Supabase email/password authentication</p>
              </div>
              <button onClick={() => setAuthOpen(false)} className="rounded-full border border-white/10 p-2 text-zinc-300 transition hover:text-white"><X size={18} /></button>
            </div>
            <div className="mt-5 space-y-4">
              {authMode === 'register' ? <BasicInput label="Full name" value={authForm.fullName} onChange={(v) => setAuthForm((prev) => ({ ...prev, fullName: v }))} /> : null}
              <BasicInput label="Email" value={authForm.email} onChange={(v) => setAuthForm((prev) => ({ ...prev, email: v }))} type="email" />
              <BasicInput label="Password" value={authForm.password} onChange={(v) => setAuthForm((prev) => ({ ...prev, password: v }))} type="password" />
              {authStatus ? <p className="text-sm text-zinc-400">{authStatus}</p> : null}
              <div className="flex flex-wrap gap-3">
                <button onClick={handleAuthSubmit} className="inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-zinc-100 px-5 py-3 text-sm text-black transition hover:opacity-90">{authMode === 'login' ? 'Log in' : 'Create account'}</button>
                <button onClick={() => setAuthMode((prev) => prev === 'login' ? 'register' : 'login')} className="inline-flex items-center gap-2 rounded-2xl border border-white/10 px-5 py-3 text-sm text-zinc-100 transition hover:border-white/30 hover:text-white">{authMode === 'login' ? 'Need an account?' : 'Already have an account?'}</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {accountOpen ? (
        <div className="fixed inset-0 z-[76] flex items-end justify-center bg-black/70 p-2 sm:p-4 backdrop-blur-sm md:items-center" onClick={() => setAccountOpen(false)}>
          <div className="max-h-[94vh] w-full max-w-5xl overflow-auto rounded-[2rem] border border-white/10 bg-zinc-950" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between border-b border-white/10 px-6 py-5 md:px-8">
              <div>
                <p className="text-sm font-medium uppercase tracking-[0.3em] text-white">Customer Account</p>
                <p className="mt-1 text-xs text-zinc-500">Addresses, account data, order history</p>
              </div>
              <div className="flex items-center gap-3">
                <button onClick={handleLogout} className="inline-flex items-center gap-2 rounded-2xl border border-white/10 px-4 py-2 text-sm text-zinc-200 transition hover:border-white/30 hover:text-white"><LogOut size={16} /> Logout</button>
                <button onClick={() => setAccountOpen(false)} className="rounded-full border border-white/10 p-2 text-zinc-300 transition hover:text-white"><X size={18} /></button>
              </div>
            </div>
            <div className="grid gap-6 p-4 sm:p-6 md:grid-cols-[0.95fr_1.05fr] md:gap-8 md:p-8">
              <div className="space-y-6">
                <div className="rounded-3xl border border-white/10 bg-black/30 p-5">
                  <p className="mb-4 text-sm font-medium uppercase tracking-[0.24em] text-white">Saved address</p>
                  <div className="grid gap-4 md:grid-cols-2">
                    <BasicInput label="Label" value={addressForm.label} onChange={(v) => setAddressForm((prev) => ({ ...prev, label: v }))} />
                    <BasicInput label="Recipient" value={addressForm.recipient_name} onChange={(v) => setAddressForm((prev) => ({ ...prev, recipient_name: v }))} />
                    <BasicInput label="Phone" value={addressForm.phone} onChange={(v) => setAddressForm((prev) => ({ ...prev, phone: v }))} />
                    <BasicInput label="Country" value={addressForm.country} onChange={(v) => setAddressForm((prev) => ({ ...prev, country: v }))} />
                    <BasicInput label="State / Region" value={addressForm.state_region} onChange={(v) => setAddressForm((prev) => ({ ...prev, state_region: v }))} />
                    <BasicInput label="City" value={addressForm.city} onChange={(v) => setAddressForm((prev) => ({ ...prev, city: v }))} />
                    <BasicInput label="Postal Code" value={addressForm.postal_code} onChange={(v) => setAddressForm((prev) => ({ ...prev, postal_code: v }))} />
                  </div>
                  <div className="mt-4"><BasicTextarea label="Address line 1" value={addressForm.address_line1} onChange={(v) => setAddressForm((prev) => ({ ...prev, address_line1: v }))} rows={2} /></div>
                  <div className="mt-4"><BasicTextarea label="Address line 2" value={addressForm.address_line2} onChange={(v) => setAddressForm((prev) => ({ ...prev, address_line2: v }))} rows={2} /></div>
                  <label className="mt-4 flex items-center gap-2 text-sm text-zinc-300"><input type="checkbox" checked={addressForm.is_default} onChange={(e) => setAddressForm((prev) => ({ ...prev, is_default: e.target.checked }))} /> Set as default</label>
                  <button onClick={saveAddress} className="mt-4 inline-flex items-center gap-2 rounded-2xl border border-white/10 bg-zinc-100 px-5 py-3 text-sm text-black transition hover:opacity-90">Save address</button>
                </div>
              </div>
              <div className="space-y-6">
                <div className="rounded-3xl border border-white/10 bg-black/30 p-5">
                  <div className="flex items-center justify-between gap-3">
                    <p className="text-sm font-medium uppercase tracking-[0.24em] text-white">Address book</p>
                    <button onClick={() => loadAccountData()} className="rounded-2xl border border-white/10 px-3 py-2 text-xs text-zinc-200">Refresh</button>
                  </div>
                  <div className="mt-4 space-y-3">
                    {accountLoading ? <p className="text-sm text-zinc-500">Loading...</p> : null}
                    {(accountData.addresses || []).map((addr) => (
                      <div key={addr.id} className="rounded-2xl border border-white/10 p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div className="text-sm text-zinc-200">
                            <p className="font-medium text-white">{addr.label}{addr.is_default ? ' · default' : ''}</p>
                            <p className="mt-1">{addr.recipient_name}</p>
                            <p className="mt-1 text-zinc-400">{addr.postal_code} {addr.state_region} {addr.city}</p>
                            <p className="mt-1 text-zinc-400">{addr.address_line1} {addr.address_line2}</p>
                          </div>
                          <button onClick={() => deleteAddress(addr.id)} className="rounded-2xl border border-red-500/20 px-3 py-2 text-xs text-red-300">Delete</button>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="rounded-3xl border border-white/10 bg-black/30 p-5">
                  <p className="text-sm font-medium uppercase tracking-[0.24em] text-white">Order history</p>
                  <div className="mt-4 space-y-3">
                    {(accountData.orders || []).length === 0 ? <p className="text-sm text-zinc-500">No orders yet.</p> : null}
                      ...
                      <div key={order.id} className="rounded-2xl border border-white/10 p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            ...
                            <p className="mt-2 text-xs text-zinc-400">{(order.order_items || []).map((item) => `${item.sku} × ${item.quantity}`).join(' · ')}</p>
                          </div>
                          <div className="text-right">
                            <p className="text-sm text-zinc-100">{formatYen(order.total_jpy || 0)}</p>
                            <p className="mt-1 text-xs text-zinc-500">{order.status} / {order.fulfillment_status}</p>
                          </div>
                        </div>
                      </div>
                    ))}
